...
DIM iDebug_start AS GLOBAL LONG
DIM iDebug_stop AS GLOBAL LONG
DIM iDebug_save AS GLOBAL INTEGER
DIM iDBOut AS GLOBAL LONG
DIM iObs AS GLOBAL LONG
'
...
DIM PART_I AS GLOBAL DOUBLE        ' hardness intercept for the relation PART_A = H * PART_HM + pH * PART_PM + DOC * PART_DM + PART_I
DIM PART_HM AS GLOBAL DOUBLE       ' hardness slope for the relation PART_A = H * PART_HM + pH * PART_PM + DOC * PART_DM + PART_I or PART_A = H * PART_HM + PART_HI
DIM PART_PM AS GLOBAL DOUBLE       ' pH slope of the relation PART_A = H * PART_HM + pH * PART_PM + DOC * PART_DM + PART_I
DIM PART_DM AS GLOBAL DOUBLE       ' DOC slope of the relation PART_A = H * PART_HM + pH * PART_PM + DOC * PART_DM + PART_I
...
DIM T_Al AS GLOBAL DOUBLE 'total Al, from starting inputs in speciation mode, or adjusted every iteration before CHESS in toxicity mode
...
DIM DOC_Ads_Density AS GLOBAL DOUBLE
...
'
DIM MaxCATRows AS GLOBAL INTEGER
 MaxCATRows = 1000
DIM CAT_CA(1 TO MaxCATRows) AS GLOBAL DOUBLE ' the critical accumulation
DIM CAT_Species(1 TO MaxCATRows) AS GLOBAL STRING        ' the species name (latin or common)
DIM CAT_TestType(1 TO MaxCATRows) AS GLOBAL STRING       ' the test type (e.g. acute or chronic)
DIM CAT_Lifestage(1 TO MaxCATRows) AS GLOBAL STRING      ' lifestage
DIM CAT_Endpoint(1 TO MaxCATRows) AS GLOBAL STRING       ' endoint (e.g., survival, reproduction, growth)
DIM CAT_Quantifier(1 TO MaxCATRows) AS GLOBAL STRING     ' quantifier (e.g., EC20, EC10, etc)
DIM CAT_References(1 TO MaxCATRows) AS GLOBAL STRING     ' references
DIM CAT_Miscellaneous(1 TO MaxCATRows) AS GLOBAL STRING  ' any other comments relevant to this entry
...
DIM iCAT_NRow AS GLOBAL INTEGER                   ' the number of rows in the table (must be >0 and <MaxCATRows)
...
DIM DOELECTROSTATICS AS GLOBAL INTEGER
    DOELECTROSTATICS = FALSE
DIM WHAMSpecificBoundConc(1 TO 2, 1 TO 20, MaxSpecies) AS GLOBAL DOUBLE
DIM WHAMBidentDeprotConc(1 TO 2, 1 TO 2, 9 TO 20) AS GLOBAL DOUBLE
DIM WHAMFreeConc(1 TO 2, 1 TO 20) AS GLOBAL DOUBLE
DIM WHAMDiffuseBoundConc(1 TO 2, MaxSpecies) AS GLOBAL DOUBLE
