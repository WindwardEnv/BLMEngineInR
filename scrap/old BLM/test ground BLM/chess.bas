...
'IF (iDebug = 09) THEN CALL DebugLog ("CHES", "After TempCorrection, OKT/KT(iAl)="+STR$(OKT(iAl)) + " / " + STR$(KT(iAl)))
...
'IF (iDebug = 09) THEN CALL DebugLog ("CHES", "After SSaturate, OKT/KT(iAl)="+STR$(OKT(iAl)) + " / " + str$(KT(iAl)))
...
'IF (iDebug = 09) THEN CALL DebugLog ("CHES", "After FirstGuess, OKT/KT(iAl)="+STR$(OKT(iAl)) + " / " + STR$(KT(iAl)))

IF ErrorCode% THEN
   CALL MessageFile(iEC_Critical&, iObs&, ErrorMsg$)
   IF (iDebug = 01 OR iDebug = 07) THEN CALL DebugLog ("CHES", "ErrorCode Exit from CHESS")
   EXIT SUB
END IF
...
'IF (iDebug = 09) THEN CALL DebugLog ("CHES", "After CalcSpeciesConc, OKT/KT(iAl)="+STR$(OKT(iAl)) + " / " + STR$(KT(iAl)))
...
DO
   Iter% = Iter% + 1
   iSing% = FALSE
   IF (iDebug = 01 OR iDebug = 07) THEN CALL DebugLog ("CHES", "Start CHESS Iteration " + STR$(Iter%))
   ...
   IF NOT Skip% THEN
      ...
      IF iSing THEN
         ...
         ErrorCode% = TRUE
         IF (iDebug = 01 OR iDebug = 07) THEN CALL DebugLog ("CHES", "ErrorCode Exit from CHESS - singluar matrix")
         EXIT SUB
      END IF
   END IF
   ...
   IF (Iter% > MaxIter%) THEN
      ...
      IF (iDebug = 01 OR iDebug = 07) THEN CALL DebugLog ("CHES", "ErrorCode Exit from CHESS - iterations exceed max")
      EXIT SUB
   END IF

LOOP UNTIL (Converge% AND NOT Skip%)
IF (iDebug = 01 OR iDebug = 07) THEN CALL DebugLog ("CHES", "After CHESS iterations have ended.")
      ...
      '        Call Pmsg(Label(iLA50Variable))
            ...
            CASE "CRITICAL START"
               ...
               DO
                  ...
                  IF iCAT_NRow > MaxCATRows THEN
                     ' Too many rows in the CAT
                     ErrorCode% = TRUE
                     ErrorMsg$ = "Too many rows in critical accumulation table."
                     EXIT SUB
                  END IF
...
SUB WHAMCalcDiffuse (HS%)
    IF (iDebug = 01) THEN CALL DebugLog ("WHAM", "Begin WHAMCalcDiffuse")

    '##############################################################################
    WWSR7:     ' called by WWSR5                                                 ##
    '##          calcs binding by DL accumulation, for FA and HA                 ##
    '##############################################################################
    IF (iDebug = 01) THEN CALL DebugLog ("WHAM", "WWSR7")
    ...
    IF NOT DOELECTROSTATICS THEN GOTO WWSR7L5
    ...
    WWSR7L2:
    'IF (iDebug = 01) THEN CALL DebugLog ("WHAM", "WWSR7L2")
    ...
    WWSR7L1:
    'IF (iDebug = 01) THEN CALL DebugLog ("WHAM", "WWSR7L1")
    ...
    FOR X% = 1 TO NSP%
        ...
        TDCONCCALC = TDCONCCALC + (wCDDL(HS%, X%) * wCH%(X%))
    WWSR7L4:
    'IF (iDebug = 01) THEN CALL DebugLog ("WHAM", "WWSR7L4")
    NEXT X%
    '
    ...
    WWSR7L5:
    'IF (iDebug = 01) THEN CALL DebugLog ("WHAM", "WWSR7L5")
    'RETURN
    IF (iDebug = 01) THEN CALL DebugLog ("WHAM", "End WHAMCalcDiffuse")
END SUB

SUB WHAMCalcSpecies
'STATIC wISTR!
IF (iDebug = 01) THEN CALL DebugLog ("WHAM", "Begin WHAMCalcSpecies")
...
'##############################################################################
IF (iDebug = 01) THEN CALL DebugLog ("WHAM", "WWSR5")
...
WWSR5L1:
'IF (iDebug = 01) THEN CALL DebugLog ("WHAM", "WWSR5L1")
...
WWSR5L2:
'IF (iDebug = 01) THEN CALL DebugLog ("WHAM", "WWSR5L2")
NEXT X%
...
WWSR5L3:
'IF (iDebug = 01) THEN CALL DebugLog ("WHAM", "WWSR5L3")
NEXT HS%
...
WWSR5L4:
'IF (iDebug = 01) THEN CALL DebugLog ("WHAM", "WWSR5L4")
NEXT HS%
...
WWSR5L5:
'IF (iDebug = 01) THEN CALL DebugLog ("WHAM", "WWSR5L5")

IF (iDebug = 01) THEN CALL DebugLog ("WHAM", "End WHAMCalcSpecies")

END SUB

SUB WHAMCalcSpecific (HS%)
   IF (iDebug = 01) THEN CALL DebugLog ("WHAM", "Begin WHAMCalcSpecific")

'##############################################################################
WWSR6:     ' called by WWSR5                                                 ##
'##          calcs NU (specific binding) and Z for FA, HA                    ##
'##############################################################################
IF (iDebug = 01) THEN CALL DebugLog ("WHAM", "WWSR6")
...
WWSR6L1:
'IF (iDebug = 01) THEN CALL DebugLog ("WHAM", "WWSR6L1")
   NEXT k%
...
WWSR6L2:
'IF (iDebug = 01) THEN CALL DebugLog ("WHAM", "WWSR6L2")
   NEXT k%
   ...
   TMP = (1 - wFP(SYTE1%)) * (1 - wFP(SYTE2%)) * (1 - SUMBITHETA)

   BINETCH = BIMETCH + PROT2CH + PROT1CH1 + PROT1CH2 - 2      ' net charge
   wBIZ(j%) = BINETCH * wSM(HS%, j%)                            ' this site's charge

   WHAMSpecificBoundConc(HS%, j% + 8, CMap%(1)) = (PROT2CH / 2) * wSM(HS%, j%) * wTH(HS%)
   WHAMBidentDeprotConc(HS%, 1, j% + 8) = PROT1CH1 * wSM(HS%, j%) * wTH(HS%)
   WHAMBidentDeprotConc(HS%, 2, j% + 8) = PROT1CH2 * wSM(HS%, j%) * wTH(HS%)
   WHAMFreeConc(HS%, j% + 8) = TMP * wSM(HS%, j%) * wTH(HS%)
...
FOR k% = 1 TO NSP%
    ...
    FOR j% = 1 TO 12
        ...
        IF Label$(CMap%(k%)) <> "" THEN
            WHAMSpecificBoundConc(HS%, j% + 8, CMap%(k%)) = wBISNU(j%, k%) * wTH(HS%)
        END IF
    NEXT j%

WWSR6L3:
'IF (iDebug = 01) THEN CALL DebugLog ("WHAM", "WWSR6L3")
NEXT k%
...
FOR j% = 1 TO 8
    ...
    WWSR6L4:
    'IF (iDebug = 01) THEN CALL DebugLog ("WHAM", "WWSR6L4")
    NEXT k%
    ...
    WWSR6L5:
    'IF (iDebug = 01) THEN CALL DebugLog ("WHAM", "WWSR6L5")
    NEXT k%
    ...
    WHAMSpecificBoundConc(HS%, j%, CMap%(1)) = PROT1CH * wSP(HS%, j%) * wTH(HS%)
    WHAMFreeConc(HS%, j%) = WHAMSpecificBoundConc(HS%, j%, CMap%(1)) * (1 / wFP(j%) - 1)
...
FOR k% = 1 TO NSP%
    ...
    FOR j% = 1 TO 8
        ...
        IF Label$(CMap%(k%)) <> "" THEN
            WHAMSpecificBoundConc(HS%, j%, CMap%(k%)) = wMONSNU(j%, k%) * wTH(HS%)
        END IF
    NEXT j%

WWSR6L6:
'IF (iDebug = 01) THEN CALL DebugLog ("WHAM", "WWSR6L6")
NEXT k%
...
WWSR6L7:
'IF (iDebug = 01) THEN CALL DebugLog ("WHAM", "WWSR6L7")
NEXT k%
...
IF (iDebug = 01) THEN CALL DebugLog ("WHAM", "End WHAMCalcSpecific")

END SUB

SUB WHAMDefine
   IF (iDebug = 01) THEN CALL DebugLog ("WHAM", "Begin WHAMDefine")
    ...
    '##          sets up Model V                                                 ##
    '##############################################################################
    IF (iDebug = 01) THEN CALL DebugLog ("WHAM", "WWSR1")
    ...
    FOR i% = 1 TO NSP%
        ...
        WWSR1L1:
        'IF (iDebug = 01) THEN CALL DebugLog ("WHAM", "WWSR1L1")
    ...
    WWSR1L2:
    'IF (iDebug = 01) THEN CALL DebugLog ("WHAM", "WWSR1L2")
    NEXT i%

    'RETURN

    IF (iDebug = 01) THEN CALL DebugLog ("WHAM", "End WHAMDefine")

END SUB

SUB WHAMIter
'##############################################################################
IF (iDebug = 01) THEN CALL DebugLog ("WHAM", "Begin WHAMIter")
IF (iGlobalAl = 2) THEN
  ' Al is always controlled by solubility
  IF (iDebug = 01) THEN CALL DebugLog ("WHAM", "iGlobalAl = 2")
  iALPPT = TRUE
  LKSOAL25 = 8.625
  'adj up to slightly higher Al solubility as suggested by Niva data
  LKSOAL25 = 9.76
...
IF (iGlobalFe = 2) THEN
  ...
  IF (iDebug = 01) THEN CALL DebugLog ("WHAM", "iGlobalFe = 2")
  iFePPT = TRUE
  LKSOFE25 = 3.0
    LKSOFE25 = 1.0
    LKSOFE25 = 2.5
...
IF (iGlobalPb = 2) THEN
  ...
  IF (iDebug = 01) THEN CALL DebugLog ("WHAM", "iGlobalPb = 2")
  iPbPPT = TRUE
'  LKSOFE25 = 3.0
'    LKSOFE25 = 1.0
'    LKSOFE25 = 2.5
...
IF (iGlobalCO2=TRUE) THEN
   IF (iDebug = 01) THEN CALL DebugLog ("WHAM", "iGlobalCO2 = TRUE")
...
IF (iDebug = 01) THEN CALL DebugLog ("WHAM", "WWSR2")
...
WWSR2L1:
'IF (iDebug = 01) THEN CALL DebugLog ("WHAM", "WWSR2L1")
...
WWSR2L2:
'IF (iDebug = 01) THEN CALL DebugLog ("WHAM", "WWSR2L2")
...
WWSR2L3:
'IF (iDebug = 01) THEN CALL DebugLog ("WHAM", "WWSR2L3")
...
WWSR2L4:
'IF (iDebug = 01) THEN CALL DebugLog ("WHAM", "WWSR2L4")
...
FOR i = 1 TO NSP%
   ...
   IF (ii <> 0) THEN
      ...
      WHAMDiffuseBoundConc(1, ii) = DVOL(1) * wCDDL(1, i)
      WHAMDiffuseBoundConc(2, ii) = DVOL(2) * wCDDL(2, i)
   END IF
NEXT i
'RETURN
...
'##############################################################################
IF (iDebug = 01) THEN CALL DebugLog ("WHAM", "WWSR3")
...
WWSR3L1:
'IF (iDebug = 01) THEN CALL DebugLog ("WHAM", "WWSR3L1")
...
WWSR3L2:
'IF (iDebug = 01) THEN CALL DebugLog ("WHAM", "WWSR3L2")
NEXT X%
...
WWSR3L3:
'IF (iDebug = 01) THEN CALL DebugLog ("WHAM", "WWSR3L3")
NEXT X%

IF iCO2PPT THEN
    ' CO3 is controlled by CO2 solubility
    IF (iDebug = 01) THEN CALL DebugLog ("WHAM", "Control CO2 with solubility")
    LKH_CO2 = -1.5
    'pPCO2 = -3.5
    Log_K_H2CO3 = 16.681
    Log_CO3 = pPCO2 + LKH_CO2 + 2*pH - Log_K_H2CO3
    IF (iDebug = 01) THEN CALL DebugLog ("WHAM", "Log_CO3 = " + STR$(Log_CO3))
    wA(55) = 10^(Log_CO3)
...
WWSR3L4:
'IF (iDebug = 01) THEN CALL DebugLog ("WHAM", "WWSR3L4")
NEXT X%
...
FOR X% = 52 TO 100                                              ' anions
   ...
   IF wT(X%) = 0 THEN GOTO WWSR3L5
      ...
      IF CONCERR > CRIT THEN GOTO WWSR3L1                                  ' re-iterate
WWSR3L5:
'IF (iDebug = 01) THEN CALL DebugLog ("WHAM", "WWSR3L5")
NEXT X%
...
WWSR3L6:
'IF (iDebug = 01) THEN CALL DebugLog ("WHAM", "WWSR3L6")
RETURN
...
'##############################################################################
IF (iDebug = 01) THEN CALL DebugLog ("WHAM", "WWSR4")
...
WWSR8L1:
'IF (iDebug = 01) THEN CALL DebugLog ("WHAM", "WWSR8L1")
NEXT X%
...
IF (iDebug = 01) THEN CALL DebugLog ("WHAM", "End WHAMIter")
...
IF (iDebug = 01 OR iDebug = 04) THEN CALL DebugLog ("CSpe", "After Calculate Activity Coefficients, ISTR " + STR$(ISTR#))
...
FOR q = 1 TO NPhase
   ...
   IF PType(P) = ptSaturated THEN
       ...
       IF (iDebug = 01 OR iDebug = 04) THEN CALL DebugLog ("CSpe", "Replacing concentrations with solubility expressions, OC."+Label(s) +"="+STR$(OC(s)))
   END IF
NEXT q
IF (iDebug = 01 OR iDebug = 04) THEN CALL DebugLog ("CSpe", "After replacing concentrations with solubility expressions")
...
FOR S = NFirstS TO NSpecies
 ...
 END IF
NEXT S
IF (iDebug = 01 OR iDebug = 04) THEN CALL DebugLog ("CSpe", "After temperature and solubility corrections")

'CALL WHAMCalcSpecies
IF (NOT iDoneWHAM) AND (iWHAM) THEN
   IF (iDebug = 01 OR iDebug = 04) THEN CALL DebugLog ("CSpe", "Calling WHAMIter")
   CALL WHAMIter
   iDoneWHAM = TRUE
END IF
IF (iDebug = 01 OR iDebug = 04) THEN CALL DebugLog ("CSpe", "After Call WHAMIter")


IF (iAl <> 0) AND (iAlp <> 0) THEN
   IF (iDebug = 01 OR iDebug = 04) THEN CALL DebugLog ("CSpe", "Before Call CalcAlSurfCharge")
   CALL CalcAlSurfCharge
   IF (iDebug = 01 OR iDebug = 04) THEN CALL DebugLog ("CSpe", "After Call CalcAlSurfCharge")
END IF

IF FALSE THEN
   IF (iAl <> 0) AND (iAlp <> 0) THEN
     IF (iDebug = 01 OR iDebug = 04) THEN CALL DebugLog ("CSpe", "Before KT(iAlp) correction")
     ...
     END IF
     'OC(iAlp) = KT(iAlp)
     IF (iDebug = 01 OR iDebug = 04) THEN CALL DebugLog ("CSpe", "After KT(iAlp) correction")
   END IF
END IF
...
SUB CalcAlSurfCharge'(LC50 as integer)

  ' Local variables
  DIM T_Al_part AS DOUBLE
  DIM T_Al_diss AS DOUBLE

  T_Al_diss = KT(iAl)
  IF T_Al > T_Al_diss THEN
     T_Al_part = T_Al - T_Al_diss
  ELSE
     T_Al_part = OKT(iAlp)
  END IF
  IF LC50 = FALSE THEN
     ...
     IF (iDebug = 01 OR iDebug = 09) THEN DebugLog("CalcAlSurfCharge", "After solubility calc, updating Part. Al")
  END IF
  IF (iDebug = 01 OR iDebug = 09) THEN DebugLog("CalcAlSurfCharge", "After solubility calc, Total Al (mol/L) = " + STR$(T_Al))
  IF (iDebug = 01 OR iDebug = 09) THEN DebugLog("CalcAlSurfCharge", "After solubility calc, Part. Al (OKT/KT) (mol/L) = " + STR$(KT(iAlp)) + " / " + STR$(KT(iAlp)))
  IF (iDebug = 01 OR iDebug = 09) THEN DebugLog("CalcAlSurfCharge", "After solubility calc, Diss. Al (OKT/KT) (mol/L) = " + STR$(KT(iAl)) + " / " + STR$(KT(iAl)))

  ' DOC Charge & DOC
  DOC_mgL = OKT(iDOC) * 1000! / 2   'DOC (mg C/L)
  DOC_Charge_eqL = ((ZED(1) * PercHA! * KT(iDOC)) + (ZED(2) * (1! - PercHA!) * KT(iDOC))) ' DOC Charge (eq/L) = HA Charge (eq/L) + FA Charge (eq/L)
  IF (iDebug = 01 OR iDebug = 09) THEN DebugLog("CalcAlSurfCharge", "After solubility calc, ZED(HA) = " + STR$(ZED(1)))
  IF (iDebug = 01 OR iDebug = 09) THEN DebugLog("CalcAlSurfCharge", "After solubility calc, ZED(FA) = " + STR$(ZED(2)))
  IF (iDebug = 01 OR iDebug = 09) THEN DebugLog("CalcAlSurfCharge", "After solubility calc, DOC Charge (eq/L) = " + STR$(DOC_Charge_eqL))

  ' Adsorption Density (mgC / molAl) = 10^(-1.1229 - 0.935955 * log10(T_Al_diss + T_Al_part) + 0.8829 * log10(DOC_mgL) +  0.5443 * pH - 0.04964 * pH^2)
  DOC_Ads_Density = -1.1229  'Intercept
  DOC_Ads_Density = DOC_Ads_Density - 0.935955 * LOG10(T_Al) 'Total Al effects
  DOC_Ads_Density = DOC_Ads_Density + 0.8829 * LOG10(DOC_mgL) 'DOC effects
  DOC_Ads_Density = DOC_Ads_Density + 0.5443 * pH 'linear pH effects
  DOC_Ads_Density = DOC_Ads_Density - 0.04964 * pH^2  'quadratic pH Effects
  DOC_Ads_Density = 10 ^ DOC_Ads_Density
  IF (iDebug = 01 OR iDebug = 09) THEN DebugLog("CalcAlSurfCharge", "After solubility calc, DOC (mg/L) = " + STR$(DOC_mgL))
  IF (iDebug = 01 OR iDebug = 09) THEN DebugLog("CalcAlSurfCharge", "After solubility calc, pH = " + STR$(pH))
  IF (iDebug = 01 OR iDebug = 09) THEN DebugLog("CalcAlSurfCharge", "After solubility calc, Adsorption Density (mg C/mol Al) = " + STR$(DOC_Ads_Density))

  ' Adsorbed DOC charge (eq / mol Al) = [Adsorption Density (mg C / mol Al)] * [DOC Charge (eq/L)] / [DOC (mg C/L)]
  DOC_Surf_Charge = (DOC_Ads_Density * DOC_Charge_eqL) / DOC_mgL
  IF (iDebug = 01 OR iDebug = 09) THEN DebugLog("CalcAlSurfCharge", "After solubility calc, Adsorbed DOC Charge (eq/mol Al) = " + STR$(DOC_Surf_Charge))

  ' Inorganic Surface Charge (eq/mol Al) = Sum(charge of Alp species)
  Inorg_Surf_Charge = 0 'Inorganic Surface Charge (eq/mol Al)
  FOR i_SC = 1 TO NSpecies
     IF (SType(i_SC) = stPrecipitate) AND (tSCa(i_SC, iAlp) <> 0) THEN
         Inorg_Surf_Charge = Inorg_Surf_Charge + OC(i_SC) * V(i_SC)
     END IF
  NEXT i
  Inorg_Surf_Charge = Inorg_Surf_Charge / T_Al_part
  IF (iDebug = 01 OR iDebug = 09) THEN DebugLog("CalcAlSurfCharge", "After solubility calc, Inorganic Surface Charge (eq/mol Al) = " + STR$(Inorg_Surf_Charge))

  ' Total Surface Charge (eq/mol Al) = [Adsorbed DOC Charge (eq/mol Al)] + [Inorganic Surface Charge (eq/mol Al)]
  Surf_Charge = Inorg_Surf_Charge + DOC_Surf_Charge
  IF (iDebug = 01 OR iDebug = 09) THEN DebugLog("CalcAlSurfCharge", "After solubility calc, Total Surface Charge (eq/mol Al) = " + STR$(Surf_Charge))

  ' Effects from Current level of particulate effects
  Surf_Charge_Al_i = (T_Al_part * Surf_Charge) * 1000000
  IF (iDebug = 01 OR iDebug = 09) THEN DebugLog("CalcAlSurfCharge", "iCalcLA50Al, Surface Charge * Al (ueq/L) = " + STR$(Surf_Charge_Al_i))

END SUB'CalcAlSurfCharge
