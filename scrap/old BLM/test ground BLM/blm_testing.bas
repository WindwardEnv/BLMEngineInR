...
DECLARE FUNCTION ASINH(X AS DOUBLE) AS DOUBLE
...
DECLARE SUB AdjustCu2 (Diff#, Iter%)
DECLARE SUB AdjustCu3 (Diff#, Iter%)
DECLARE SUB AdjustCu4 (Diff#, Iter%)
DECLARE SUB AdjustAlp (Diff#, Iter%)
...
DECLARE SUB GetArgs (Arg1$, Arg2$, Arg3$, LC50ALL%, iDeriv%, iExpert1%, iExpert2%, iExpert3%, iExpert4%, ErrorCode%, ErrorMsg$)
...
DECLARE SUB WriteDeriv (iHand%, iObs&, a#(), DLC50#(), n%)
DECLARE SUB WriteDetailed (iHand%, iObs&)
DECLARE SUB WriteSimple (iHand%, iObs&, LC50%)
DECLARE SUB WriteDetailed2 (iHand&, iObs&, iComplete)
DECLARE SUB WriteSimple2 (iHand&, iObs&, LC50, iComplete)
DECLARE SUB WriteDetailedComma (iHand&, iObs&, iComplete)
DECLARE SUB WriteSimpleComma (iHand&, iObs&, LC50, iComplete)
...
VersionID$ = "ver 2.3.0, build 2014-07-03"      'now uses last free ion solution as first guess for next run, added
                                                 '/B2 switch for output log BLM_LA50 for use in optimization
VersionID$ = "ver 2.3.0, build 2014-07-07"      'changed total values in detailed xls
VersionID$ = "ver 2.3.1, build 2014-07-28"      'changed variable type for NData from integer to longint to allow more
                                                 'than 32767 input observations
...
VersionID$ = "2.37, build 2015-10-22"           'revised precipitated Al normalization approach, removed particulate
                                                 'species from output when not Al, added WQC, FMB, and /V switches
                                                 '(merge with blm_212g)
VersionID$ = "2.38, build 2016-02-28"           'Fixed problem with solubility switches
VersionID$ = "2.39, build 2016-06-15"           'Fixed the problem with using the low hardness adjustment in expert
                                                 'toxicity mode, made changes to prevent problems with "Iterations exceed max"
...
VersionID$ = "2.40, build 2016-12-21"           'Added capability to use a critical accumulation table (CAT) in the
                                                 'parameter file.  This is a table that has the critical accumation
                                                 'along with organism & test information and references
VersionID$ = "2.40, build 2017-01-16"           'Added error handling to opening the BLM input file.  If the file
                                                 'cannot be accessed, it will give an error code.
VersionID$ = "2.41, build 2017-09-22"           'Added iteration cap to LC50 finding, and started implementing a
                                                 'message file for communication with the interface.
VersionID$ = "2.42, build 2017-11-27"           'Corrected a bug in the /CA switch.  In 2.41, a number > 10 after this
                                                 'switch would be ignored and the first value would be used.
VersionID$ = "2.43, build 2018-03-29"           'Updated the /CO2 switch so that it could be specified as /CO2x, where
                                                 'x is the pCO2 to use for the open system.  Will still default to 3.5
                                                 'if not specified
VersionID$ = "2.43, build 2018-04-01"           'Got the comma-separated output format working (/O2 switch).
VersionID$ = "2.44, build 2018-05-04"           'Fixed the /CO2 switch - it was cutting off the last number when the
                                                 'switch was put at the end of the arguments list.
VersionID$ = "2.44, build 2018-09-12"           'Fixed error with the FMB not working with files that are not the EPA
                                                 'Cu WQC file -  set defaults of 1 for ACUTE_DIV and CHRONIC_DIV, but
                                                 'have not enabled yet.
VersionID$ = "2.44, build 2018-10-30"           'Fixed error where CMC/CCC and TU's are calculated for incomplete data
                                                 'as well, with just the last complete observation being subbed in,
                                                 'enabled defaults for ACUTE_DIC and CHRONIC_DIV
...
VersionID$ = "2.45, build 2019-02-28"           'When using the /B3 switch, it will now give -999 for the two values
                                                 'if it's an incomplete observation.
VersionID$ = "2.46, build 2019-04-30"           'WORK IN PROGRESS. Added in a new species type for precipiated species
                                                 'so that surface complexation reactions can be performed with Al.
                                                    ' Fixed GetArgs so that /KSALXXX gets eliminated from the string properly.
VersionID$ = "2.46, build 2019-05-02"           'WORK IN PROGRESS. Fixed GetArgs so /PB2 doesn't leave "2" in the
                                                 'command string.
VersionID$ = "2.46, build 2019-06-24"           'WORK IN PROGRESS. working on getting the precipitated Al effects
                                                 'working with surface charge considered.
VersionID$ = "2.46, build 2019-07-01"           'WORK IN PROGRESS. fixed problem with calculation of inorganic surface
                                                 'charge in precipitated Al calculations.
VersionID$ = "2.46, build 2019-07-02"           'WORK IN PROGRESS. added the hardness relationships back into
                                                 'precipitated Al calculations
VersionID$ = "2.46, build 2019-07-08"           'WORK IN PROGRESS. fixed mistake in precipitated Al formulation and
                                                 'declaration of global variables
VersionID$ = "2.46, build 2019-07-09"           'WORK IN PROGRESS. Changed the Al-DOC adsorption density equation so
                                                 'that the predictions fit the data better.
VersionID$ = "2.46, build 2019-09-05"           'WORK IN PROGRESS. Re-set the number of iterations (ii) to 1 when we
                                                 'find that both dissolved and precipitated effects are needed.
                                                 'This is a new optimization, and it needs to be communicated as
                                                 'such to AdjustAlp, otherwise it will try to run with OKT(iAlp) = 0
                                                 'and never converge in CHESS.
VersionID$ = "2.46, build 2019-09-06"           'WORK IN PROGRESS. Fixed problem where iPartMe was set to FALSE when
                                                 'DISS_A and PART_HI were not in the parameter file - they can just be
                                                 'in the extra columns of the input file when using the /E3 switch.
VersionID$ = "2.46, build 2019-09-25"           'WORK IN PROGRESS. Updated the speciation calculation for Al so that
                                                 'it will give you a "first guess" of DISS_A and PART_HI, assuming that
                                                 'the user has given it the total Al in both iAl and iAlp slots,
                                                 'and that full amount of total Al is what's needed. There is a
                                                 'wrapper program in progress for using this output and optimizing the
                                                 'DISS_A and PART_HI parameters to give a better auto-validation result.
VersionID$ = "2.46, build 2019-09-26"           'WORK IN PROGRESS. Minor changes on the road to debugging.
VersionID$ = "2.46, build 2019-12-17"           'Added a fourth expert mode for Al, where the extra columns correspond
                                                 'to TARGET_R, LogKSAL, DISS_A, and PART_HI, respectively, for toxicity
                                                 'mode, and just TARGET_R and logKSAL for speciation mode.
VersionID$ = "2.46, build 2019-12-30"           'set the "Dissolved and precipitated Al required for target effect"
                                                 'message to not appear when /QQ is set
VersionID$ = "2.46, build 2020-05-06"           'debugging - trying to figure out why the FMB doesn't work when you do
                                                 'chronic FMB anymore
VersionID$ = "2.47, build 2020-05-07"           'Fixing problem with FMB not working in Chronic FMB only mode. Change
                                                 'of version number.
VersionID$ = "2.48, build 2021-01-13"           'Debugging problem with Al toxicity mode where the balance between
                                                 'particulate and dissolved Al was not working - if you increased the
                                                 'KSAL really high, the model was still putting a lot of particulate Al
                                                 'into the solution, even though plenty of dissolved Al should have been
                                                 'available for reaching the ECX. This was fixed by making a variable that
                                                 'checks during the first iteration if the dissolved Al at the solubility
                                                 'limit is enough to reach the ECX.  If not (Diff < 0 when ii = 1) then
                                                 'the iNeedPartMe is set to TRUE, and the solubility limit of Al is used
                                                 'for the dissolved and the model loops to adjust the particulate amount.
                                                 'Otherwise, the particulate Al will be ignored and the dissolved will be
                                                 'adjusted like normal. Also changed the check of KSAL so it will give a
                                                 'warning message but will not throw an error and exit.
VersionID$ = "2.48, build 2021-06-16"           'Adjustments related to debugging the Al model - including output of surface
                                                 'charge value (was only outputting Surf_Charge_Al not Surf_Charge_Al_i to
                                                 'comma and XLS files). Fixed problem where the OutputMask variable was
                                                 'getting messed up after the HA/FA charge in the WriteDetailed2 sub. Fixed
                                                 'the label for "Al Surf. Charge" column header running into the column
                                                 'header before it. Added surface charge information to the detailed files.
                                                 'Fixed instances of missing "iCol = iCol + 1" in the WriteDetailedXLS sub.
VersionID$ = "2.48, build 2021-06-29"           'Consolidated all Al surface charge calculations to the CalcAlSurfCharge sub
                                                 'in chess.bas. This function is now called in CalcSpeciesConc after WHAMIter
                                                 'so that CalcResidual might be a bit more accurate. Made some adjustments to
                                                 'how the particulate vs dissolved Al is reported in speciation mode, and the
                                                 'surface charge estimate should be a lot closer between toxicity and
                                                 'speciation mode now.
VersionID$ = "2.48, build 2021-06-30"           'Added PART_PM and PART_DM parameters for Al toxicity, replaced PART_HI with PART_I.
VersionID$ = "2.48, build 2021-07-08"           'Reverted back to not include PART_DM, PART_PM, and back to PART_HI. Packaging with
                                                 'software to send to client.
VersionID$ = "2.48, build 2021-07-09"           'So, yeah, um...why did we make the relation with surface charge logarithmic? If there's
                                                 'a negative surface charge, it causes problems...so I set a minimum of 1E-10 for the
                                                 'surface charge in the equation in the speciation calculation. Hopefully that'll be
                                                 'good enough.
VersionID$ = "2.49, build 2021-09-01"           'We're trying a new MLR for the PART_A estimation...basically we're using a hyperbolic
                                                 'arcsine transformation on the surface charge in ueq/L...yes, it must be
                                                 'microequivalents per liter specifically. That will then be fed into a NON-LOG version
                                                 'of the dose-response equation...hopefully this will work.
VersionID$ = "2.49, build 2021-10-25"           'Changed data type of iObs, i, and NData to all be long integers instead of integers.
VersionID$ = "2.49, build 2021-10-31"           'debugged problem with iObs/Iter not registering correctly in the WriteSimple[...]
                                                 'and WriteDetailed[...] functions.
VersionID$ = "2.49, build 2021-11-04"           'Re-enabled PART_DM and PART_PM slopes.
VersionID$ = "2.49, build 2022-02-23"           'AdjustCu4 is now based on log concentrations rather than linear space. This was an
                                                 'attempt to fix a problem where the /A4 switch resulted in the program getting stuck
                                                 'in the WHAMIter loop for some random concentrations (that are only slightly different
                                                 'from the concentrations used in other AdjustCu subs) when in the upper ranges that
                                                 'you sometimes see when searching for bracketing concentrations. This change seemed to
                                                 'fix the problem, but the engine is running very slow with this switch (e.g., /A4 switch
                                                 'takes 17 seconds to complete ~170 obs, with about 6 AdjustCu iterations, vs 6 seconds
                                                 'for the same data file using the /A1 switch and taking 16 AdjustCu iterations...this
                                                 'makes no sense!). Another change was made that allows the specifying of a start and
                                                 'stop observation to use the debug switch on, so now can specify /Gxx[:start[:stop]],
                                                 'where[] indicates an optional argument.
VersionID$ = "2.49, build 2022-03-23"           'Increased the max number of rows in the CAT table to 1000.
VersionID$ = "2.49, build 2022-03-29"           'Made AdjustCu, AdjustCu2, and AdjustCu3 based on log concentrations as well. Also
                                                 'decreased the number of times that the log10 function was called in all 4 AdjustCu
                                                 'functions to try to make it go faster.
VersionID$ = "2.49, build 2022-03-30"           'Elimiated the log-space AdjustCu(2/3/4) changes because they will definitely make things
                                                 'converge slower. Changed the step for the bracketing search to be 5^-SignDiff rather than
                                                 'base-10 so we're taking smaller steps and hopefully won't end up overshooting as badly in
                                                 'some cases, which will hopefully prevent the freezing problem. Added code to update the
                                                 'low bound with the previous high bound when the bracketing search is done and the high bound
                                                 'is now above 0 (and vice-versa) - this was making the root-finding start from 2 orders of
                                                 'magnitude apart rather than 1.
VersionID$ = "2.49, build 2022-04-10"           'Changed the bracketing search step to be 2^-SignDiff rather than 5, since I found at least one
                                                 'case where it got stuck again.
VersionID$ = "2.49, build 2022-04-11"           'Changed bracketing search to using false position, bounded by a factor of 10 in all 4 AdjustCu procedures.
VersionID$ = "2.50, build 2023-05-16"           'OKT(iMetal) will be set at a minimum value of 1E-20 if doing a toxicity run because
                                                 'otherwise the model has trouble converging. This is especially a problem when the first
                                                 'observation is given as 9.99e-99, as happens when there is a missing value in a file saved from
                                                 'the UI software.
VersionID$ = "2.50, build 2023-06-15"           'Added additional communication with the interface with MessageFile...any time there is an
                                                 'error code, the ErrorMsg value will be written out...Not sure yet if the times I did this in
                                                 'CHESS will be needed or excessive.
VersionID$ = "2.50X, build for test only"
'VersionID$ = "2.49, build XXXX-XX-XX"
...
'
DIM i AS LONG 'INTEGER           ' Loop counter
    i = 0
DIM ii AS INTEGER              'toxicity iteration counter
'DIM LC50 AS INTEGER        ' Boolean flag when TRUE we are predicting LC50 -changed to global variable in all_dims.bi
    LC50 = FALSE
...
DIM iExpert3 AS INTEGER     ' Flag to determine expert mode
    iExpert3 = FALSE
DIM iExpert4 AS INTEGER     ' Flag to determine expert mode
    iExpert4 = FALSE
...
'
CALL GetArgs(Arg1$, Arg2$, Arg3$, LC50ALL, iDeriv, iExpert1, iExpert2, iExpert3, iExpert4, ErrorCode%, ErrorMsg$)
IF ErrorCode% THEN
   CALL MessageFile(iEC_Critical&, 0, ErrorMsg$)
...
IF (iDebug > 0) THEN
   ...
   IF (iDebug = 01) OR (iDebug = 10) THEN DebugLog("GetArgs", "iCalc_FMB = " + STR$(iCalc_FMB))
   IF (iDebug = 01) OR (iDebug = 10) THEN DebugLog("GetArgs", "iCriteria = " + STR$(iCriteria))
   IF (iDebug = 01) OR (iDebug = 10) THEN DebugLog("GetArgs", "FMB_Option = " + STR$(FMB_Option))
END IF
...
IF (DISS_B = 0) OR (PART_B = 0) OR (LC50ALL = FALSE) OR ((PART_A = 0) AND (PART_HM = 0)) OR ((NOT iExpert3) AND ((DISS_A = 0) OR ((PART_A = 0) AND (PART_HI = 0))))  THEN
    iPartMe = FALSE
ELSE
    iPartMe = TRUE
END IF
...
iDebug_save = iDebug
IF iDebug_stop& < 0 THEN iDebug_stop& = NData&
FOR i& = 1 TO NData&
   iObs& = i&
   IF (iObs& < iDebug_start&) OR (iObs& > iDebug_stop&) THEN
      iDebug = 00
   ELSE
      iDebug = iDebug_save
   END IF
   ...
   CALL GetData2 (iInFile, LC50, iExpert1, iExpert2, iExpert3, iExpert4, PFile$, CriticalValue, ErrorCode%, ErrorMsg$)
   IF (iDebug = 01 OR iDebug = 02) THEN CALL DebugLog ("main", "After GetData2")
   IF (iDebug = 01) THEN DebugLog("main", "OKT(iAl) = " + STR$(OKT(iAl)))
   IF (iDebug = 01) THEN DebugLog("main", "OKT(iAlp) = " + STR$(OKT(iAlp)))

   ' save initial total Al
   IF (iAl <> 0) AND (iAlp <> 0) THEN
      IF ABS(1-OKT(iAlp)/OKT(iAl)) < 0.00001 THEN
         T_Al = OKT(iAl)'in speciation mode, we input the total Al in both Al and Alp columns
      ELSE
         T_Al = OKT(iAl) + OKT(iAlp)'but, let's give the option of giving diss and part separately
      END IF
   END IF
   ...
   END IF
   IF (iExpert4 = TRUE) THEN
     TARGET_R = TARGET_R_expert
     LogKsAl = LogKsAl_expert
   END IF
...
'               TARGET_R = VAL(TmpS3$)
   ...
   DO 'LOOP UNTIL NDeriv = ZERO%
      ...
      IF (LC50 = TRUE) AND (LastLC50# > 0) THEN
         OKT(iMetal) = LastLC50#
      END IF
      IF (LC50 = TRUE) AND (OKT(iMetal) < 1E-20) THEN
          OKT(iMetal) = 1E-20
      END IF
      '
      '
      ...
      DO 'LOOP UNTIL (NOT LC50) OR BLCrit OR NOT iComplete
       ...
       '  OC(1) = OKT(1)
         ...
         IF (iComplete = TRUE) THEN
            ...
            'IF (iAl <> 0) AND (iAlp <> 0) THEN CALL CalcAlSurfCharge(LC50)
            IF (iDebug = 01 OR iDebug = 02) THEN CALL DebugLog ("main", "After CHESS, Iter" + STR$(ii))
            TIX END CycleCount&&
            IF (iDebug = 01 OR iDebug = 02) THEN CALL DebugLog ("main", "CHESS Elapsed Time = " + STR$(CycleCount&&))
            ...
            IF (LC50 = TRUE) THEN
               ...
               IF (iPartMe = TRUE) THEN' AND (ii = 1)
                  ...
                  IF ii = 1 THEN OKT(iMetal) = KT(iMetal)
                  IF (Diff < 0) AND (ii = 1) THEN
                    iNeedPartMe = TRUE
                    IF (iDebug = 01 OR iDebug = 09 OR iDebug = 99) THEN CALL DebugLog("main", "After Diff, iNeedPartMe = TRUE")
                  END IF
                  IF (iNeedPartMe) THEN
                      IF Surf_Charge_Al = 0 THEN 'ii = 1
                          IF NOT iVQuiet THEN PRINT "Dissolved and precipitated Al required for target effect"
                          ii = 1
                          ...
                          DISS_Effect = 1/(1+EXP(DISS_B*LOG10(R1) - DISS_A*DISS_B))             'KEC: Why is this not DISS_Effect = 1 / (1 + EXP(DISS_A + DISS_B * LOG10(R1)), as it's written in the manuscript?
                          IF (iDebug = 01 OR iDebug = 99) THEN CALL DebugLog ("main", "After solubility calc, BLMConc = " + STR$(BLMConc!))
                          IF (iDebug = 01 OR iDebug = 99) THEN CALL DebugLog ("main", "After solubility calc, CritBL = " + STR$(CritBL))
                          IF (iDebug = 01 OR iDebug = 99) THEN CALL DebugLog ("main", "After solubility calc, Fract = " + STR$(Fract))
                          IF (iDebug = 01 OR iDebug = 99) THEN CALL DebugLog ("main", "After solubility calc, ConvCrit = " + STR$(ConvCrit))
                          IF (iDebug = 01 OR iDebug = 99) THEN CALL DebugLog ("main", "After solubility calc, DISS_Effect = " + STR$(DISS_Effect))

                          ' R_part = 1-((1-R_T)/(1-R_diss))
                          PART_Effect = 1-((1-TARGET_R)/(1-DISS_Effect))
                          'Surf_Charge_Al = 10^((LOG(1 / PART_Effect - 1) - PART_A) / PART_B)
                          Hardness_mgL = (OKT(FindSpecies%("Ca")) + OKT(FindSpecies%("Mg"))) * 1000 * 100.0086 ' hardness in mg/L
                          DOC_mgL = OKT(iDOC) * 1000! / 2   'DOC (mg C/L)
                          IF (PART_HI = 0) AND (PART_A <> 0) THEN PART_HI = PART_A 'if there's no hardness adjustment specified, just save the given PART_A as the intercept
                          'PART_A = PART_HI + LOG10(Hardness_mgL) * PART_HM
                          PART_A = SINH(PART_HI + PART_PM * pH + PART_DM * LOG10(DOC_mgL) + PART_HM * LOG10(Hardness_mgL))
                          'PART_A = PART_I + LOG10(Hardness_mgL) * PART_HM + LOG10(DOC_mgL) * PART_DM + pH * PART_PM
                          'Surf_Charge_Al = 10^((LOG(1 / PART_Effect - 1) + PART_A * PART_B) / PART_B) 'the "critical" surface charge
                          Surf_Charge_Al = (LOG(1 / PART_Effect - 1) + PART_A * PART_B) / PART_B 'the "critical" surface charge


                          IF (iDebug = 01 OR iDebug = 09) THEN DebugLog("main", "After solubility calc, R1 = " + STR$(R1))
                          IF (iDebug = 01 OR iDebug = 09 OR iDebug = 99) THEN DebugLog("main", "After solubility calc, PART_Effect = " + STR$(PART_Effect))
                          IF (iDebug = 01 OR iDebug = 09 OR iDebug = 99) THEN DebugLog("main", "After solubility calc, Hardness (mg/L) = " + STR$(Hardness_mgL))
                          IF (iDebug = 01 OR iDebug = 09 OR iDebug = 99) THEN DebugLog("main", "After solubility calc, DOC (mg/L) = " + STR$(DOC_mgL))
                          IF (iDebug = 01 OR iDebug = 09 OR iDebug = 99) THEN DebugLog("main", "After solubility calc, PART_A = " + STR$(PART_A))
                          IF (iDebug = 01 OR iDebug = 09 OR iDebug = 99) THEN DebugLog("main", "After solubility calc, PART_B = " + STR$(PART_B))
                          IF (iDebug = 01 OR iDebug = 09 OR iDebug = 99) THEN DebugLog("main", "After solubility calc, PART_HI = " + STR$(PART_HI))
                          IF (iDebug = 01 OR iDebug = 09 OR iDebug = 99) THEN DebugLog("main", "After solubility calc, PART_PM = " + STR$(PART_PM))
                          IF (iDebug = 01 OR iDebug = 09 OR iDebug = 99) THEN DebugLog("main", "After solubility calc, PART_DM = " + STR$(PART_DM))
                          IF (iDebug = 01 OR iDebug = 09 OR iDebug = 99) THEN DebugLog("main", "After solubility calc, PART_HM = " + STR$(PART_HM))
                          IF (iDebug = 01 OR iDebug = 09 OR iDebug = 99) THEN DebugLog("main", "After solubility calc, Surf_Charge_Al = " + STR$(Surf_Charge_Al))

                          IF (iDebug = 01 OR iDebug = 09) THEN DebugLog("main", "After solubility calc, pH = " + STR$(pH)) 'this doesn't change
                          IF (iDebug = 01 OR iDebug = 09) THEN DebugLog("main", "After solubility calc, ISTR = " + STR$(OC(NSpecies + 2)))'this doesn't really change

                      END IF


                      Diff = Surf_Charge_Al_i - Surf_Charge_Al
                      'Diff = (10^(Surf_Charge_Al_i*10^9)) - Surf_Charge_Al
                      'Fract = ABS(Diff) / Surf_Charge_Al
                      Fract = ABS(DIFF) / ABS(Surf_Charge_Al)
                      Part_Conc = KT(iAlp) * (1000000*26.981)
                      IF (iDebug = 01 OR iDebug = 09 OR iDebug = 99) THEN DebugLog("main", "After solubility calc, Surf_Charge_Al_i (ueq/L) = " + STR$(Surf_Charge_Al_i))
                      'IF (iDebug = 01 OR iDebug = 09 OR iDebug = 99) THEN DebugLog("main", "After solubility calc, 10^(Surf_Charge_Al_i * 10^9) = " + STR$(10^((10^9) * Surf_Charge_Al_i)))
                      IF (iDebug = 01 OR iDebug = 09) THEN DebugLog("main", "After solubility calc, Diff = " + STR$(Diff))
                      IF (iDebug = 01 OR iDebug = 09 OR iDebug = 99) THEN DebugLog("main", "After solubility calc, Fract = " + STR$(Fract))
                      IF (iDebug = 01 OR iDebug = 09) THEN DebugLog("main", "After solubility calc, Part_Conc = " + STR$(Part_Conc))
                      IF (iDebug = 01 OR iDebug = 09) THEN DebugLog("main", "After solubility calc, OKT(iAlp) = " + STR$(OKT(iAlp)))
                      IF (iDebug = 01 OR iDebug = 09) THEN DebugLog("main", "After solubility calc, KT(iAlp) = " + STR$(KT(iAlp)))

                      TmpS$ = "Part Al,"
                      TmpS$ = TmpS$ + "HA Charge,"
                      TmpS$ = TmpS$ + "FA Charge,"
                      TmpS$ = TmpS$ + "DOC Charge,"
                      TmpS$ = TmpS$ + "ISTR,"
                      TmpS$ = TmpS$ + "Adsorption Density,"
                      TmpS$ = TmpS$ + "Adsorbed DOC Charge,"
                      TmpS$ = TmpS$ + "Inorganic Surface Charge,"
                      TmpS$ = TmpS$ + "Total Surface Charge,"
                      TmpS$ = TmpS$ + "Surf_Charge_Al_i"
                      IF (iDebug = 98 AND iObs = 1 AND ii = 1) THEN DebugLog("main", "After solubility calc, " + TmpS$)

                      TmpS$ = STR$(KT(iAlp)) + "," 'Part Al
                      TmpS$ = TmpS$ + STR$(ZED(1) * PercHA! * KT(iDOC)) + "," 'HA Charge
                      TmpS$ = TmpS$ + STR$(ZED(2) * (1! - PercHA!) * KT(iDOC)) + "," 'FA Charge
                      TmpS$ = TmpS$ + STR$(CSNG(ZED(1) * PercHA! * KT(iDOC)) + CSNG(ZED(2) * (1! - PercHA!) * KT(iDOC))) + "," 'DOC Charge
                      TmpS$ = TmpS$ + STR$(OC(NSpecies + 2)) + "," 'ISTR
                      TmpS$ = TmpS$ + STR$(DOC_Ads_Density) + "," 'Adsorption density
                      TmpS$ = TmpS$ + STR$(DOC_Surf_Charge) + "," 'Adsorbed DOC Charge
                      TmpS$ = TmpS$ + STR$(Inorg_Surf_Charge) + "," 'Inorganic Surface Charge
                      TmpS$ = TmpS$ + STR$(Surf_Charge) + "," 'Total Surface Charge
                      TmpS$ = TmpS$ + STR$(Surf_Charge_Al_i) 'Surf_Charge_Al_i
                      IF (iDebug = 98 AND ii = 1) THEN DebugLog("main", "After solubility calc, " + TmpS$)
...
'                      PRINT " Diff"; Diff
              ...
              END IF 'IF (iPartMe = TRUE) AND (ii = 1)
               ...
               IF (Fract < ConvCrit) OR (ErrorNotConverged = TRUE) THEN
                  ...
                  IF (iDebug = 01 OR iDebug = 09 OR iDebug = 99) THEN DebugLog("main", "Convereged on Solution, Diff = " + STR$(Diff))
                  IF (iDebug = 01 OR iDebug = 09 OR iDebug = 99) THEN DebugLog("main", "Convereged on Solution, Surf_Charge_Al_i = " + STR$(Surf_Charge_Al_i))
                  'IF (iDebug = 01 OR iDebug = 09 OR iDebug = 99) THEN DebugLog("main", "Convereged on Solution, 10^(Surf_Charge_Al_i*10^9) = " + STR$(10^(Surf_Charge_Al_i*10^9)))
                  IF (iDebug = 01 OR iDebug = 09 OR iDebug = 99) THEN DebugLog("main", "Convereged on Solution, OKT(iMetal) = " + STR$(OKT(iMetal)))
                  IF (iDebug = 01 OR iDebug = 09 OR iDebug = 99) THEN DebugLog("main", "Convereged on Solution, OKT(iAlp) = " + STR$(OKT(iAlp)))
                  IF (iDebug = 01 OR iDebug = 09 OR iDebug = 99) THEN DebugLog("main", "Convereged on Solution, KT(iAlp) = " + STR$(KT(iAlp)))

               ELSEIF iPartMe AND (Surf_Charge_Al <> 0) THEN
                    CALL AdjustAlp(Diff, ii)
                    ii = ii + 1
                    ' update total Al
                    T_Al = OKT(iAl) + OKT(iAlp)
                    IF (iDebug = 01 OR iDebug = 09 OR iDebug = 99) THEN DebugLog("main", "After AdjustAlp, OKT(iAlp) = " + STR$(OKT(iAlp)))
                    IF (iDebug = 01 OR iDebug = 09 OR iDebug = 99) THEN DebugLog("main", "After AdjustAlp, KT(iAlp) = " + STR$(KT(iAlp)))
               ELSE
                  '
                  ' Update the total copper concentration
                  IF (iDebug = 01) OR (iDebug = 90) THEN DebugLog("main","Before AdjustCu, OKT(iMetal) = " + STR$(OKT(iMetal)) + ", Diff = " + STR$(Diff))
                  SELECT CASE iAdjust
                     CASE ONE
                     ...
                     CASE ELSE
                        CALL AdjustCu4(Diff, ii)
                  END SELECT
                  IF (iDebug = 01) OR (iDebug = 90) THEN DebugLog("main","After AdjustCu, OKT(iMetal) = " + STR$(OKT(iMetal)) + ", Diff = " + STR$(Diff))
...
'                      PRINT "OC(Al)"; OC(iMetal)
      ...
      IF (iDeriv = FALSE) OR (NDeriv = iBL) THEN
         ...
         IF iCriteria <> ZERO THEN
            HC5(i) = OKT(iMetal)
            IF (iDebug = 01) OR (iDebug = 10) THEN DebugLog("main", "HC5(i) = " + STR$(HC5(i)))
            'PRINT HC5(i)
      ...
      IF (iCalcLA50) THEN
        ...
        IF iCalcLA50Al AND NOT LC50ALL THEN
          ...
          '
            ...
            ' Calc PART_A assuming all of ECx is due to precip Al*SC
            '   PART_A = LOG10(ECx)-(1/B)*LN((1/(Rx))-1)
            Tmp1 = LOG10(MAX(1E-10, Surf_Charge_Al_i))-(1/PART_B)* LOG((1/TARGET_R)-1)
            'Tmp1 = LOG10(10^(Surf_Charge_Al_i*10^9))-(1/PART_B)* LOG((1/TARGET_R)-1)
            Hardness_mgL = (KT(FindSpecies%("Ca")) + KT(FindSpecies%("Mg"))) * 1000 * 100 ' hardness in mg/L
            DOC_mgL = OKT(iDOC) * 1000! / 2   'DOC (mg C/L)
            ' Calculate PART_HI = PART_A - Log10(H)*PART_HM
            'Tmp3 = Tmp1 - LOG10(Hardness_mgL)*PART_HM
            'Tmp3 = Tmp1 - LOG10(Hardness_mgL)*PART_HM - LOG10(DOC_mgL)*PART_DM - pH*PART_PM
            Tmp3 = ASINH(Tmp1) - LOG10(Hardness_mgL) * PART_HM - LOG10(DOC_mgL) * PART_DM - pH * PART_PM
            IF (iDebug = 01 OR iDebug = 09) THEN DebugLog("main", "iCalcLA50Al, Hardness (mg/L) = " + STR$(Hardness_mgL))
            IF (iDebug = 01 OR iDebug = 09) THEN DebugLog("main", "iCalcLA50Al, Surf_Charge_Al_i (ueq/L) = " + STR$(Surf_Charge_Al_i))
            IF (iDebug = 01 OR iDebug = 09) THEN DebugLog("main", "iCalcLA50Al, PART_HI = " + STR$(Tmp3))
            IF NOT iComplete THEN
                Tmp3 = -999.
            END IF
...
IF Iter = 1 THEN
   ...
   CuLow = OKT(iMetal)
   CuHigh = OKT(iMetal)
   'CuLow = OldTCu'log10(OKT(iMetal))
   'CuHigh = OldTCu'log10(OKT(iMetal))
   BLLow = Diff
   BLHigh = Diff
   Bracket = FALSE
...
'      CALL Pmsg(adT$)
   IF (Iter > 10) THEN
      ...
      'OKT(iMetal) = 10^((CuLow + CuHigh) / 2)
   ELSE
      OKT(iMetal) = c - d / ddc
      'NewTCu = c - d / ddc
      'OKT(iMetal) = 10^(NewTCu)
      IF (OKT(iMetal) < CuLow) OR (OKT(iMetal) > CuHigh) THEN
      'IF (NewTCu < CuLow) OR (NewTCu > CuHigh) THEN
         ' New estimate is bad, revert to false position
         OKT(iMetal) = CuLow + (CuHigh - CuLow) * BLLow / (BLLow - BLHigh)
         'OKT(iMetal) = 10^(CuLow + (CuHigh - CuLow) * BLLow / (BLLow - BLHigh))
      END IF
   END IF
ELSE
'
' We don't bracket, keep expanding the range until we do
      IF (SignDiff < 0) THEN
         ...
         BLHigh = Diff
         CuHigh = OKT(iMetal)
         FP_tmp = CuLow + (CuHigh - CuLow) * BLLow / (BLLow - BLHigh) * 1.1
         step_lim = 10 * OKT(iMetal)
         IF (FP_tmp > step_lim) OR (FP_tmp <= ZeroD) OR (Iter = 1) THEN
            OKT(iMetal) = step_lim
         ELSE
            OKT(iMetal) = FP_tmp
         END IF
      ...
      ''
      'SignDiff = Sign(Diff)
      'OKT(iMetal) = OKT(iMetal) * (2 ^ -SignDiff)
END IF
...
'      CALL Pmsg(adT$)
   ...
   ELSE
      ...
      'OKT(iMetal) = 10^cnew
      IF (OKT(iMetal) < CuLow) OR (OKT(iMetal) > CuHigh) THEN
      'IF (cnew < CuLow) OR (cnew > CuHigh) THEN
         ' New estimate is bad, revert to false position
         OKT(iMetal) = CuLow + (CuHigh - CuLow) * BLLow / (BLLow - BLHigh)
         'OKT(iMetal) = 10^(CuLow + (CuHigh - CuLow) * BLLow / (BLLow - BLHigh))
      END IF
...
' We don't bracket, keep expanding the range until we do
      ...
      IF (SignDiff < 0) THEN
         ...
         BLHigh = Diff
         CuHigh = OKT(iMetal)
         FP_tmp = CuLow + (CuHigh - CuLow) * BLLow / (BLLow - BLHigh) + 0.1 * (CuHigh - CuLow)
         step_lim = 10 * OKT(iMetal)
         IF (FP_tmp > step_lim) OR (FP_tmp <= ZeroD) OR (Iter = 1) THEN
            OKT(iMetal) = step_lim
         ELSE
...
'      CALL Pmsg(adT$)
   ...
   ELSE
      ...
      'OKT(iMetal) = 10^(cnew * 1e-6)
      IF (OKT(iMetal) < CuLow) OR (OKT(iMetal) > CuHigh) THEN
      'IF ((cnew * 1e-6) < CuLow) OR ((cnew * 1e-6) > CuHigh) THEN
         ' New estimate is bad, revert to false position
         OKT(iMetal) = CuLow + (CuHigh - CuLow) * BLLow / (BLLow - BLHigh)
         'OKT(iMetal) = 10^(CuLow + (CuHigh - CuLow) * BLLow / (BLLow - BLHigh))
      END IF
...
SUB AdjustAlp (Diff, Iter)
  STATIC AlpHigh AS DOUBLE
  STATIC AlpLow AS DOUBLE
  STATIC SCHigh AS DOUBLE
  STATIC SCLow AS DOUBLE
  STATIC Bracket AS INTEGER
  STATIC iDebugOld AS INTEGER
  ...
  ' Check the current value of total metal and the SC-metal produced
  ' against the previous high and low values
  IF (Iter = 1 OR Iter = 50 OR Iter = 100 OR Iter = 150 OR Iter = 200) THEN
     ...
     'iDebugOld = FREEFILE
     'OPEN "debug.prn" FOR OUTPUT AS #iDebugOld
     'PRINT #iDebugOld, " TAlp      "; " Next TAlp    "; " Diff     "; " AlpHigh   "; " SCHigh "; " AlpLow    "; " SCLow"
     ' BEGIN DEBUG
     'INPUT j
  END IF

  ' Do we bracket yet?
  IF ((NOT Bracket) AND (SignDiff <> Sign(SCLow))) THEN
     '
     ' Great!, we bracket now set the flag so won't keep checking
     Bracket = TRUE
  END IF
  ...
  ' We bracket, so use false position
     ...
     ELSE
        ...
        AlpHigh = OKT(iAlp)
     END IF
     OKT(iAlp) = AlpLow + (AlpHigh - AlpLow) * SCLow / (SCLow - SCHigh)
     IF (iDebug = 01 OR iDebug = 09 OR iDebug = 99) THEN DebugLog("AdjustAlp", "Bracketing")
  ELSE
  '
  ' We don't bracket, keep expanding the range until we do
     IF (SignDiff < 0) THEN
     ...
     '
     SignDiff = Sign(Diff)
     OKT(iAlp) = OKT(iAlp) * (10 ^ -SignDiff)
     IF (iDebug = 01 OR iDebug = 09 OR iDebug = 99) THEN DebugLog("AdjustAlp", "Not Bracketing")
  END IF

  IF (iDebug = 01 OR iDebug = 09 OR iDebug = 99) THEN CALL DebugLog ("AdjustAlp", "Iter:  "+STR$(Iter))
  IF (iDebug = 01 OR iDebug = 09 OR iDebug = 99) THEN CALL DebugLog ("AdjustAlp", "AlpLow:  "+STR$(AlpLow))
  IF (iDebug = 01 OR iDebug = 09 OR iDebug = 99) THEN CALL DebugLog ("AdjustAlp", "AlpHigh: "+STR$(AlpHigh))
  IF (iDebug = 01 OR iDebug = 09 OR iDebug = 99) THEN CALL DebugLog ("AdjustAlp", "SCLow:  "+STR$(SCLow))
  IF (iDebug = 01 OR iDebug = 09 OR iDebug = 99) THEN CALL DebugLog ("AdjustAlp", "SCHigh: "+STR$(SCHigh))
  IF (iDebug = 01 OR iDebug = 09 OR iDebug = 99) THEN CALL DebugLog ("AdjustAlp", "NextMe: "+STR$(OKT(iAlp)))

  ' BEGIN DEBUG
  'PRINT #iDebugOld, USING "##.###^^^^"; OldTAlp; OKT(iAlp); Diff; AlpHigh; SCHigh; AlpLow; SCLow
  ' END DEBUG
...
SUB AdjustCu (Diff, Iter)
   ...
   STATIC OldSignDiff AS INTEGER

   DIM Slope AS SINGLE
   DIM SignDiff AS INTEGER

   DIM OldTCu AS DOUBLE
   DIM TmpD AS DOUBLE
   DIM TmpD2 AS DOUBLE
   DIM FP_tmp AS DOUBLE
   DIM step_lim AS DOUBLE
   ...
   ' We bracket, so use false position
      ...
      'PRINT "    CuLow=" + STR$(CuLow)
      'PRINT "    CuHigh=" + STR$(CuHigh)
      'PRINT "    BLLow=" + STR$(BLLow)
      'PRINT "    BLHigh=" + STR$(BLHigh)


      'OKT(iMetal) = CuLow + (CuHigh - CuLow) * BLLow / (BLLow - BLHigh)
      TmpD = CuLow + (CuHigh - CuLow) * BLLow / (BLLow - BLHigh)'false position
      'TmpD = (CuLow + CuHigh) / 2'bisection
      'PRINT "    TmpD=" + STR$(TmpD)
      'TmpD = EXP(TmpD)
      'TmpD = 10 ^ TmpD
      OKT(iMetal) = TmpD
   ...
   'PRINT "    New CuLow=" + STR$(CuLow)
   'PRINT "    New CuHigh=" + STR$(CuHigh)
   'PRINT "    New BLLow=" + STR$(BLLow)
   'PRINT "    New BLHigh=" + STR$(BLHigh)
   'PRINT "    New OKT(iMetal)=" + STR$(OKT(iMetal))
...
END FUNCTION
SUB GetArgs (Arg1$, Arg2$, Arg3$, LC50ALL, iDeriv, iExpert1, iExpert2, iExpert3, iExpert4, ErrorCode%, ErrorMsg$)
   '
   ' SUB GetArgs
   '
   ...
   '      4 requires four additional columns for Al toxicity - first DISS_A, second PART_HI, third TARGET_R, and fourth KSAL
   '
   ...
   '
   ' /Gxx[:start[:stop]] - Debug mode - can turn all debug switches on or only for individual
   '   code sections. Can optionaly specify a start and stop observation.
   ...
   '      09 - only debug Al-related stuff
   '      10 - only debug FMB-related stuff
   '   Examples:
   '      /G02 - turn on main debug statements for all observations
   '      /G01:10 - turn on all debug statements starting with observation 10
   '      /G01:1:5 - turn on all debug statements for the first 5 observations
   '      /G07:50:50 - turn on CHESS debug statements for observation 50 only
   '
...
IF (p1% <> 0) THEN
  ...
  PRINT "Log KsAl should be between 7 and 11. Current value is "+ STR$( LogKsAl)
  '    EXIT SUB
  END IF
  MID$(T$, p1%, LEN(TmpS) + 5) = SPACE$(LEN(TmpS) + 5)
END IF
...
IF (p1% <> 0) THEN
  '
  '  Aluminum will always be controlled by solubility
  '
  iGlobalAl = 2
  MID$(T$, p1%, 4) = "    "
END IF
...
IF (p1% <> 0) THEN
   '
   '  Open system
   '  Inorganic carbon will always be controlled by CO2 solubility
   '
   iGlobalCO2 = TRUE
   ' the value past the CO2 switch is the pPCO2 = -log10(P_CO2, atm)
   p2% =  p1% + 4
   TmpS = MID$(T$,p2%,1)
...
IF (p1% <> 0) THEN
  '
  '  Fourth type of "expert mode" with Al toxicity parameters in
  '  four extra columns for toxicity mode and two extra for speciation
  '
  IF LC50All OR (INSTR(T$, "/L") <> 0) THEN
      iExpert3 = TRUE
  END IF
  iExpert4 = TRUE
  iLogKsAl = TRUE
...
IF (p1% <> 0) THEN
  ...
  IF (iCAT_Row <=0) OR (iCAT_Row >= MaxCATRows) THEN
      PRINT "Reverting to first row of CA Table."
      iCAT_Row = 1
  END IF
...
IF (p1% <> 0) THEN
  ...
  ' The G switch will have a number (one- or two-digit) for the debug mode, and possibly a start and/or stop observation, separated by ":"
   p2% =  p1% + 2
   TmpS = MID$(T$,p2%,1)
   IF (TmpS$ = " ") OR (TmpS$ = "/") OR (p2% >= LEN(T$)) THEN
      ' For this condition, we know that the switch /G was used with no numbers,
      ' and this is the equivalent of turning on all debug code for all observations
      '      01 - turn all debug statements on (alternate)
      p2% = p2% - 1
      iDebug = 01
   ELSE
      ' The /G switch was used with a debug type
      ' The switch should have been used with a two-digit number, but a one-digit
      ' number followed by a space or the end of the line should work too, so
      '      00 - turn all debug statements off (default)
      '      01 - turn all debug statements on
      '      02 - only debug main
      '      03 - only debug FirstGuess
      '      04 - only debug CalcSpeciesConc
      '      05 - only debug SUpdate
      '      06 - only debug CalcResidual
      '      07 - only debug CHESS main sub
      '      08 - only debug output files
      '      09 - only debug Al dose response info
      '      10+ - temporary debug calls (for whatever is specifically being tested, defined by the one coding it)
      DO UNTIL (MID$(T$,p2%,1)=" ") OR (MID$(T$,p2%,1)="/") OR (MID$(T$,p2%,1)=":") OR p2%>LEN(T$)
         p2% =  p2% + 1
      LOOP
      TmpS$ = TRIM$(MID$(T$, p1% + 2, p2%-p1%-2))
      iDebug = VAL(TmpS$)

      TmpS = MID$(T$,p2%,1)
      IF (TmpS$ = " ") OR (TmpS$ = "/") OR (p2% >= LEN(T$)) THEN
         ' The /G switch was only used with the debug type specified
         iDebug_start = 1
         iDebug_Stop = -1'temporary value until we know NObs
      ELSE
         ' The /G switch was used with the debug type + starting iteration
         p3% = p2%
         p2% = p2% + 1
         DO UNTIL (MID$(T$,p2%,1)=" ") OR (MID$(T$,p2%,1)="/") OR (MID$(T$,p2%,1)=":") OR (p2%>LEN(T$))
            p2% =  p2% + 1
         LOOP
         TmpS$ = TRIM$(MID$(T$, p3% + 1, p2%-p3% - 1))
         iDebug_start = VAL(TmpS$)

         TmpS = MID$(T$,p2%,1)
         IF (TmpS$ = " ") OR (TmpS$ = "/") OR (p2% >= LEN(T$)) THEN
            ' the /G switch was used with the debug type + starting iteration only (no stopping iteration)
            iDebug_Stop = -1'temporary value until we know NObs
         ELSE
            ' The /G switch was used with debug type + starting iteration + stopping iteration
            p3% = p2%
            DO UNTIL (MID$(T$,p2%,1)=" ") OR (MID$(T$,p2%,1)="/") OR (p2%>LEN(T$))
               p2% =  p2% + 1
            LOOP
            TmpS$ = TRIM$(MID$(T$, p3% + 1, p2%-p3% - 1))
            iDebug_Stop = VAL(TmpS$)
         END IF
      END IF
   END IF
   MID$(T$, p1%, p2%-p1%+1) = SPACE$(p2%-p1%+1)
...
END IF
IF iDebug > 0 THEN
   IF iDebug_stop < 0 THEN
      IF iDebug_start = 1 THEN
         PRINT "iDebug = " + STR$(iDebug) + " for all iterations"
      ELSE
         PRINT "iDebug = " + STR$(iDebug) + " for iteration " + STR$(iDebug_start) + " to end"
      END IF
   ELSE
      PRINT "iDebug = " + STR$(iDebug) + " for iteration " + STR$(iDebug_start) + " to " + STR$(iDebug_stop)
   END IF
END IF
...
SUB GetData2 (iInFile, LC50, iExpert1, iExpert2, iExpert3, iExpert4, PFile$, CriticalValue, ErrorCode%, ErrorMsg$)
'
'
' Read in a line of input data and determine if we are
' predicting LC50 (copper = -999)
...
IF iExpert4 THEN MaxFields = MaxFields + 2
...
IF iExpert4 THEN
   ' Check to see if there is are two additional columns
    IF (j+1 > NFields) THEN
       ErrorCode% = TRUE
       ErrorMsg$ = "Must include columns for TARGET_R and logKsAl in the input file to run in expert mode 4."
       EXIT SUB
    END IF

    TARGET_R_expert = VAL(DField$(j))
    IF TARGET_R_expert > 1 THEN
        TARGET_R_expert = TARGET_R_expert / 100
    END IF
    j = j + 1

    LogKsAl_expert = VAL(DField$(j))
    j = j + 1

    IF (LogKsAl_expert < 7) OR (LogKsAl_expert > 11) THEN
    '   ErrorCode% = TRUE
    '   ErrorMsg$ = "Log KsAl should be between 7 and 11. Current value is "+ STR$(LogKsAl_expert)
    PRINT "Log KsAl should be between 7 and 11. Current value is "+ STR$(LogKsAl_expert)
    '   EXIT SUB
    END IF
...
IF (iExpert3) THEN
   ...
   IF (j+1 > NFields) THEN
      ErrorCode% = TRUE
      ErrorMsg$ = "Must include columns for DISS_A and PART_HI in the input file to run in expert mode 3 or 4."
      EXIT SUB
   END IF
...
SUB WriteDeriv (iHand, iObs&, a(), DLC50(), n)
...
END SUB
SUB WriteDetailed2 (iHand&, iObs&, iComplete)
...
IF (iObs& = 1) THEN
   ...
   ' Write label for HA and FA charge column
   L$ = "HA Charge"' (eq/L)"
   PRINT #iHand&, FormatStr$(L$, LabelMask$);
   L$ = "FA Charge"' (eq/L)"
   PRINT #iHand&, FormatStr$(L$, LabelMask$);

   ' Aluminum-specific information
   IF iLogKsAl THEN
      L$ = "Ads. DOC SC"
      PRINT #iHand&, FormatStr$(L$, LabelMask$);
      L$ = "Inorg. SC"
      PRINT #iHand&, FormatStr$(L$, LabelMask$);
      L$ = Label(iMetal) + " Surf. Ch."
      PRINT #iHand&, FormatStr$(L$, LabelMask$);
   END IF
...
'
   ...
   ' Write FA and HA charge to output file
   IF iComplete AND NOT ErrorNotConverged THEN
      Tmp# = CSNG(ZED(1) * PercHA! * KT(iDOC))
      ...
      Tmp# = CSNG(ZED(2) * (1! - PercHA!) * KT(iDOC))
      'IF (Tmp# > 9999) OR (Tmp# < 0.01) THEN
      '   OutputMask$ = OutputMask1$
      'ELSE
      '   OutputMask$ = OutputMask2$
   ...
   IF iLogKsAl THEN
      IF iComplete AND NOT ErrorNotConverged THEN
         ...
         'Inorg. SC
         Tmp# = Inorg_Surf_Charge
         PRINT #iHand&, USING$(OutputMask$, Tmp#);

         'Al Surf. Ch.
         Tmp# = Surf_Charge_Al_i
         PRINT #iHand&, USING$(OutputMask$, Tmp#);
      ELSE
         PRINT #iHand&, "               NA";
         PRINT #iHand&, "               NA";
...
SUB WriteDetailedComma (iHand&, iObs&, iComplete)
...
IF (iObs& = 1) THEN
   ...
   ' Write label for HA and FA charge
   L$ = "HA Charge (eq/L)"
   PRINT #iHand&, FormatStr$(L$, LabelMask$);
   L$ = "FA Charge (eq/L)"
   PRINT #iHand&, FormatStr$(L$, LabelMask$);
   ...
   ' Aluminum-specific information
   IF iLogKsAl THEN
      L$ = "Ads. DOC SC (eq/mol Al)"
      PRINT #iHand&, FormatStr$(L$, LabelMask$);
      L$ = "Inorg. SC (eq/mol Al)"
      PRINT #iHand&, FormatStr$(L$, LabelMask$);
      L$ = Label(iMetal) + " Surf. Ch. (ueq/L)"
      PRINT #iHand&, FormatStr$(L$, LabelMask$);
   END IF
...
'
   ...
   ' Aluminum-specific information
   IF iLogKsAl THEN
      IF iComplte AND NOT ErrorNotConverged THEN
         'Ads. DOC SC
         Tmp# = DOC_Surf_Charge
         IF (Tmp# > 9999) OR (Tmp# < 0.01) THEN
            OutputMask$ = OutputMask1$
...
SUB WriteDetailedOld (iHand&, iObs&, iComplete)
...
SUB WriteDetailedXLS (iHand&, iObs&, iComplete)
...
IF (iObs& = 1) THEN
       ...
       FOR HS% = 1 TO 2
           ...
           FOR s% = 1 TO NSpecies
               IF (V(s%) > 0) AND (ActCorr(s%) <> acWHAM) AND (SType(s%) = stAqueous) THEN
                   iCol = iCol + 1
                   stat& = xlsWriteText("Donnan" + sTmp1$ + "-" + Label$(s%), iRow, iCol, %xlsFont1, %xlsLeftAlign, %xlsCellNormal, 0, iHand&)
               END IF
           NEXT s%
           FOR j% = 1 TO 20
               iCol = iCol + 1
               ...
               ELSEIF (j = 15) OR (j = 20) THEN
                   sTmp_site2$ = "7"
               END IF
               stat& = xlsWriteText("DOC-" + sTmp1$ + "_" + sTmp_site1$ + sTmp_site2$, iRow, iCol, %xlsFont1, %xlsLeftAlign, %xlsCellNormal, 0, iHand&)
               IF (j% > 8) THEN
                   iCol = iCol + 1
                   stat& = xlsWriteText("DOC-" + sTmp1$ + "_" + sTmp_site2$ + "-" + sTmp_site1$ + "H", iRow, iCol, %xlsFont1, %xlsLeftAlign, %xlsCellNormal, 0, iHand&)
                   iCol = iCol + 1
                   stat& = xlsWriteText("DOC-" + sTmp1$ + "_" + sTmp_site1$ + "-" + sTmp_site2$ + "H", iRow, iCol, %xlsFont1, %xlsLeftAlign, %xlsCellNormal, 0, iHand&)
               END IF
               FOR s% = 1 TO NSpecies
                   IF (ActCorr(s%) <> acWHAM) AND (SType(s%) = stAqueous) AND ((s% = 1) OR (s% = 2) OR (s% = 4) OR (s% = 5) OR (s% = 109)) THEN
                       iCol = iCol + 1
                       stat& = xlsWriteText("DOC-" + sTmp1$ + "_" + sTmp_site1$ + sTmp_site2$ + "-" + Label$(s%), iRow, iCol, %xlsFont1, %xlsLeftAlign, %xlsCellNormal, 0, iHand&)
                   END IF
               NEXT s%
           NEXT j%
       NEXT HS%
   ...
   IF (iWHAM = TRUE) THEN
       FOR HS% = 1 TO 2
           FOR s% = 1 TO NSpecies
              IF (V(s%) > 0) AND (ActCorr(s%) <> acWHAM) AND (SType(s%) = stAqueous) THEN
                  iCol = iCol + 1
                  stat& = xlsWriteText("mol", iRow, iCol, %xlsFont1, %xlsLeftAlign, %xlsCellNormal, 0, iHand&)
              END IF
           NEXT s%
           FOR j% = 1 TO 20
               iCol = iCol + 1
   ...
   ' Write units label for HA and FA charge column
   iCol = iCol + 1
   sTmp1$ = "eq / L"
   stat& = xlsWriteText(sTmp1$, iRow, iCol, %xlsFont1, %xlsLeftAlign, %xlsCellNormal, 0, iHand&)
   iCol = iCol + 1
...
'
       ...
       FOR HS% = 1 TO 2
           FOR s% = 1 TO NSpecies
               IF (V(s%) > 0) AND (ActCorr(s%) <> acWHAM) AND (SType(s%) = stAqueous) THEN
                   iCol = iCol + 1
                   IF iComplete AND NOT ErrorNotConverged THEN
                       value# = WHAMDiffuseBoundConc(HS%, s%)
                       stat& = xlsWriteNumber(value#, iRow, iCol, %xlsFont0,  %xlsLeftAlign, %xlsCellNormal, 0, iHand&)
                   ELSE
                       stat& = xlsWriteText("NA", iRow, iCol, %xlsFont0, %xlsLeftAlign, %xlsCellNormal, 0, iHand&)
                   END IF
           ...
           FOR j% = 1 TO 20
               ...
               IF (j% > 8) THEN
                   iCol = iCol + 1
                   IF iComplete AND NOT ErrorNotConverged THEN
                       value# = WHAMBidentDeprotConc(HS%, 1, j%)
                       stat& = xlsWriteNumber(value#, iRow, iCol, %xlsFont0,  %xlsLeftAlign, %xlsCellNormal, 0, iHand&)
                   ELSE
                       stat& = xlsWriteText("NA", iRow, iCol, %xlsFont0, %xlsLeftAlign, %xlsCellNormal, 0, iHand&)
                   END IF
                   iCol = iCol + 1
                   IF iComplete AND NOT ErrorNotConverged THEN
                       value# = WHAMBidentDeprotConc(HS%, 2, j%)
                       stat& = xlsWriteNumber(value#, iRow, iCol, %xlsFont0,  %xlsLeftAlign, %xlsCellNormal, 0, iHand&)
                   ELSE
                       stat& = xlsWriteText("NA", iRow, iCol, %xlsFont0, %xlsLeftAlign, %xlsCellNormal, 0, iHand&)
                   END IF
               ...
               FOR s% = 1 TO NSpecies
                   IF (ActCorr(s%) <> acWHAM) AND (SType(s%) = stAqueous) AND ((s% = 1) OR (s% = 2) OR (s% = 4) OR (s% = 5) OR (s% = 109)) THEN
                       iCol = iCol + 1
                       IF iComplete AND NOT ErrorNotConverged THEN
                           value# = WHAMSpecificBoundConc(HS%, j%, s%)
                           stat& = xlsWriteNumber(value#, iRow, iCol, %xlsFont0,  %xlsLeftAlign, %xlsCellNormal, 0, iHand&)
                       ELSE
                           stat& = xlsWriteText("NA", iRow, iCol, %xlsFont0, %xlsLeftAlign, %xlsCellNormal, 0, iHand&)
                       END IF
   ...
   IF iComplete AND NOT ErrorNotConverged THEN
      icol = iCol + 1
      value# = CSNG(ZED(1) * PercHA! * KT(iDOC))
      stat& = xlsWriteNumber(value#, iRow, iCol, %xlsFont0,  %xlsLeftAlign, %xlsCellNormal, 0, iHand&)

      icol = iCol + 1
      value# = CSNG(ZED(2) * (1! - PercHA!) * KT(iDOC))
      stat& = xlsWriteNumber(value#, iRow, iCol, %xlsFont0,  %xlsLeftAlign, %xlsCellNormal, 0, iHand&)
   ELSE
      icol = icol + 1
      stat& = xlsWriteText("NA", iRow, iCol, %xlsFont0, %xlsLeftAlign, %xlsCellNormal, 0, iHand&)
   ...
   'Al-specific information
   IF iLogKsAl THEN
      IF iComplete AND NOT ErrorNotConverged THEN
         'Ads. DOC SC
         icol = iCol + 1
         value# = DOC_Surf_Charge
         stat& = xlsWriteNumber(value#, iRow, iCol, %xlsFont0,  %xlsLeftAlign, %xlsCellNormal, 0, iHand&)

         'Inorg. SC
         icol = iCol + 1
         value# = Inorg_Surf_Charge
         stat& = xlsWriteNumber(value#, iRow, iCol, %xlsFont0,  %xlsLeftAlign, %xlsCellNormal, 0, iHand&)

         'Al Surf. Ch.
         icol = iCol + 1
         value# = Surf_Charge_Al_i
         stat& = xlsWriteNumber(value#, iRow, iCol, %xlsFont0,  %xlsLeftAlign, %xlsCellNormal, 0, iHand&)
      ELSE
         icol = icol + 1
         stat& = xlsWriteText("NA", iRow, iCol, %xlsFont0, %xlsLeftAlign, %xlsCellNormal, 0, iHand&)
...
SUB WriteSimple (iHand, iObs&, LC50)
...
SUB WriteSimple2 (iHand&, iObs&, LC50, iComplete)
...
IF (iObs& = 1) THEN
   ...
   IF iLogKsAl THEN
      ...
      PRINT #iHand&, FormatStr$("mol/L", LabelMask$);
      PRINT #iHand&, FormatStr$("frac cont", LabelMask$);
      PRINT #iHand&, FormatStr$("ueq/L", LabelMask$);
   END IF
...
SUB WriteSimpleold (iHand&, iObs&, LC50, iComplete)
...
SUB WriteSimpleXLS (iHand&, iObs&, LC50, iComplete)
...
IF (iObs& = 1) THEN
   ...
   IF iLogKsAl THEN
      ...
      stat& = xlsWriteText(sTmp1$, iRow, iCol, %xlsFont1, %xlsLeftAlign, %xlsCellNormal, 0, iHand&)
      iCol = 9
      sTmp1$ = Label(iMetal) + " Surf. Charge"
      stat& = xlsWriteText(sTmp1$, iRow, iCol, %xlsFont1, %xlsLeftAlign, %xlsCellNormal, 0, iHand&)
   END IF
   ...
   IF iLogKsAL THEN
      ...
      iCol = iCol + 1 '=9
      stat& = xlsWriteText("ueq/L", iRow, iCol, %xlsFont1, %xlsLeftAlign, %xlsCellNormal, 0, iHand&)
   END IF 'iLogKsAl
...
SUB WriteSimpleComma (iHand&, iObs&, LC50, iComplete)
...
SUB WriteWQCFile(iOutF4txt AS LONG, iOutF4xls AS LONG, iObs AS LONG, NData AS LONG)
'
'  *-----------------------------------------------------------------------*
'  |                                                                       |
'  |  SUB WriteWQCFile                                                     |
...
'      xlsWriteDefaultFormats(iOutF4xls)
 ...
 END IF 'If (Iter = 1)
  ...
  'FOR j = 1 TO NData
     ...
     END IF
  'NEXT j
  IF (iObs = NData) AND (iCalc_FMB = TRUE) THEN
      ' Calculate FMB
...
'     PRINT "FMB Chron"    ; FMB_Chron
  ...
  END IF 'iObs& = NData AND iCalc_FMB = TRUE
...
'  |      FMB = 10^(Z_EF * Me_STD + LOG10(Me_Compliance))
  ...
  '  *-----------------------------------------------------------------------------*
  IF (NCen > 0) THEN
    IF (iDebug = 01) OR (iDebug = 10) THEN DebugLog("FMB", "MLE fit needed; NCen = " + STR$(NCen))
...
SUB WriteWQCFileAcute(iOutF4txt AS LONG, iOutF4xls AS LONG, iObs AS LONG, NData AS LONG)
'
'  *-----------------------------------------------------------------------*
'  |                                                                       |
...
SUB WriteWQCFileChronic(iOutF4txt AS LONG, iOutF4xls AS LONG, iObs AS LONG, NData AS LONG)
'
'  *-----------------------------------------------------------------------*
'  |                                                                       |
...
SUB WriteWQC_Acute_and_Chronic(iOutF4txt AS LONG, iOutF4xls AS LONG, iObs AS LONG, NData AS LONG)
'
'  *-----------------------------------------------------------------------*
'  |                                                                       |
'  |  SUB WriteWQCFile                                                     |
...
iRowOS  = 11
  ...
  IF (iObs& = 1) THEN
      IF (iDebug = 01) OR (iDebug = 10) THEN DebugLog("WQC", "Iter 1 - write column headings")
    '
    '   Write column headings
    '
    IF (iOFormat = THREE) OR (iCriteria <> ZERO) THEN   ' for now, let's always make a WQC file in xls format
       ...
       IF (iCriteria <> THREE) THEN
           IF (iDebug = 01) OR (iDebug = 10) THEN DebugLog("WQC", "CMC column heading")
          iCol = iCol + 1
          SELECT CASE iCriteria
             CASE ONE
                sTmp1$ = "CMC"
       ...
       IF (iCriteria = ONE) THEN
           IF (iDebug = 01) OR (iDebug = 10) THEN DebugLog("WQC", "CCC column heading")
          iCol = iCol + 1
          sTmp1$ = "CCC"
          stat& = xlsWriteText(sTmp1$, iRow, iCol, %xlsFont1, %xlsLeftAlign, %xlsCellNormal, 0, iOutF4xls)
          ' 2nd line has the units
       ...
       IF (iCriteria <> THREE) THEN
           IF (iDebug = 01) OR (iDebug = 10) THEN DebugLog("WQC", "Acute TU heading")
          iCol = iCol + 1
          sTmp1$ = "Acute Toxic Units"
          stat& = xlsWriteText(sTmp1$, iRow, iCol, %xlsFont1, %xlsLeftAlign, %xlsCellNormal, 0, iOutF4xls)
          ' 2nd line has the units
       ...
       IF (iCriteria <> TWO) THEN
           IF (iDebug = 01) OR (iDebug = 10) THEN DebugLog("WQC", "Chronic TU heading")
          iCol = iCol + 1
          sTmp1$ = "Chronic Toxic Units"
          stat& = xlsWriteText(sTmp1$, iRow, iCol, %xlsFont1, %xlsLeftAlign, %xlsCellNormal, 0, iOutF4xls)
          ' 2nd line has the units
       ...
       IF (iCalc_FMB = TRUE) THEN
           IF (iDebug = 01) OR (iDebug = 10) THEN DebugLog("WQC", "FMB Headings")
           '
           ' FMB Labels
           '
           iCol = 3
           IF (iCriteria <> THREE) THEN
               IF (iDebug = 01) OR (iDebug = 10) THEN DebugLog("WQC", "Acute FMB heading")
              iCol = iCol + 1
              iRow = 5
              IF (FMB_option = FMBMedian) THEN
                 sTmp1$ = "Acute FMB (median)"
           ...
           IF (iCriteria <> TWO) THEN
               IF (iDebug = 01) OR (iDebug = 10) THEN DebugLog("WQC", "Chronic FMB heading")
              iCol = iCol + 1
              iRow = 5
              IF (FMB_option = FMBMedian) THEN
                 sTmp1$ = "Chronic (median)"
 ...
 END IF 'IF (ITER = 1)
  ...
  'FOR j = 1 TO NData
      j = iObs&
      IF (iOFormat = THREE) OR (iCriteria <> ZERO) THEN   ' for now, let's always make a WQC file in xls format
         IF (iDebug = 01) OR (iDebug = 10) THEN DebugLog("WQC", "Writing data for Obs (j) = " + STR$(j))
         ...
         IF (iCriteria <> THREE) THEN
            ...
            ELSE
               ...
               stat& = xlsWriteText("NA", iRow, iCol, %xlsFont0, %xlsLeftAlign, %xlsCellNormal, 2, iOutF4xls)
            END IF
            IF (iDebug = 01) OR (iDebug = 10) THEN DebugLog("WQC", "WQC_Acute(j) = " + STR$(WQC_Acute(j)))
         END IF
         '
         '  CCC
         IF (iCriteria = ONE) THEN
            ...
            ELSE
               ...
               stat& = xlsWriteText("NA", iRow, iCol, %xlsFont0, %xlsLeftAlign, %xlsCellNormal, 2, iOutF4xls)
            END IF
            IF (iDebug = 01) OR (iDebug = 10) THEN DebugLog("WQC", "WQC_Chronic(j) = " + STR$(WQC_Chron(j)) + " (From Acute HC5/FAV and CHRONIC_DIV)")
         ELSEIF (iCriteria = THREE) THEN
            WQC_Chron(j) = HC5(j) * MolWt * 1.E6
            IF (iDebug = 01) OR (iDebug = 10) THEN DebugLog("WQC", "WQC_Chronic(j) = " + STR$(WQC_Chron(j)) + " (Direct from Chronic HC5)")
         END IF
         '
         '  Ambient metal concentration
         value# = AmbientMetal(j) * MolWt * 1.E6
         ...
         iCol = iCol + 1
         stat& = xlsWriteNumber(value#, iRow, iCol, %xlsFont0,  %xlsLeftAlign, %xlsCellNormal, 2, iOutF4xls)
         IF (iDebug = 01) OR (iDebug = 10) THEN DebugLog("WQC", "MetalConc(j) = " + STR$(MetalConc(j)))
         '
         '  Acute toxic units
         IF (iCriteria <> THREE) THEN
            iCol = iCol + 1
            ...
            ELSE
               ...
               stat& = xlsWriteText("NA", iRow, iCol, %xlsFont0, %xlsLeftAlign, %xlsCellNormal, 2, iOutF4xls)
            END IF
            IF (iDebug = 01) OR (iDebug = 10) THEN DebugLog("WQC", "TU_Acute(j) = " + STR$(TU_Acute(j)))
         END IF
         '
         '  Chronic toxic units
         IF  (iCriteria <> TWO) THEN
            ...
            ELSE
               ...
               stat& = xlsWriteText("NA", iRow, iCol, %xlsFont0, %xlsLeftAlign, %xlsCellNormal, 2, iOutF4xls)
            END IF
            IF (iDebug = 01) OR (iDebug = 10) THEN DebugLog("WQC", "TU_Chron(j) = " + STR$(TU_Chron(j)))
         END IF
         '
         '  Censored flag
         value_int = CenFlag(j)
         iCol = iCol + 1
         stat& = xlsWriteInteger(value_int, iRow, iCol, %xlsFont0,  %xlsLeftAlign, %xlsCellNormal, 2, iOutF4xls)
         IF (iDebug = 01) OR (iDebug = 10) THEN DebugLog("WQC", "CenFlag(j) = " + STR$(CenFlag(j)))
  ...
  'NEXT j
     ...
     IF (iObs = NData) AND (iCalc_FMB = TRUE) THEN
        ...
        IF (iDebug = 01) OR (iDebug = 10) THEN DebugLog("WQC", "Calc FMB")
       ...
       FOR i = 1 TO NData
          ...
          'if ((iCriteria <> THREE) and (TU_Acute_tmp(i) = NULL)) or ((iCriteria = THREE) and (TU_Chronic_tmp(i) = NULL)) then
          IF (HC5_tmp(i) = NULL) OR (MetalConc_tmp(i) = NULL) THEN
              IF (iDebug = 01) OR (iDebug = 10) THEN DebugLog("WQC", "NULL TU at Obs " + STR$(i))
             PRINT STR$(i)
          ELSE
             ii = ii + 1
       ...
       NEXT i
       NData = ii
       IF (iDebug = 01) OR (iDebug = 10) THEN DebugLog("WQC", "NData = " + STR$(NData))
       ...
       IF(iCriteria <> THREE) THEN
           ...
           'stat& = xlsWriteNumber(value#, iRow, iCol, %xlsFont0,  %xlsLeftAlign, %xlsCellNormal, 0, iOutF4xls)
           stat& = xlsWriteNumber(value#, iRow, iCol, %xlsFont0,  %xlsLeftAlign, %xlsCellNormal, 2, iOutF4xls)
           IF (iDebug = 01) OR (iDebug = 10) THEN DebugLog("WQC", "FMB_Acute = " + STR$(FMB_Acute))
       END IF
       ...
       IF (iCriteria <> TWO) THEN
        ...
        iCol = iCol + 1
        stat& = xlsWriteNumber(value#, iRow, iCol, %xlsFont0,  %xlsLeftAlign, %xlsCellNormal, 2, iOutF4xls)
        IF (iDebug = 01) OR (iDebug = 10) THEN DebugLog("WQC", "FMB_Chron = " + STR$(FMB_Chron))
       END IF
...
SUB PrintHelp
   ...
   PRINT "/E4 - (for Al toxicity) read TARGET_R, logKsAl, DISS_A, and PART_HI from 4 extra columns in the input file"
   PRINT "/E4 - (for Al speciation) read TARGET_R and logKsAl from 2 extra columns in the input file"
