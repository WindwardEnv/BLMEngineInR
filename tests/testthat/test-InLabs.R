test_that("AddInLabs works", {
  ...
  expect_error(AddInLabs(ThisProblem = carbonate_system_problem, InLabName = NA))
...
test_that("RemoveInLabs works", {
  ...
  expect_error(RemoveInLabs(ThisProblem = carbonate_system_problem, InLabToRemove = "NotThere"),
               regexp = "does not exist")

  # removing number that doesn't exist
  expect_error(RemoveInLabs(ThisProblem = carbonate_system_problem, InLabToRemove = 999),
               regexp = "trying to remove")

  # bad index
  expect_error(RemoveInLabs(ThisProblem = carbonate_system_problem, InLabToRemove = -1),
